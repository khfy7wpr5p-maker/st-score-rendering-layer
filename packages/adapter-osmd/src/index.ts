...
import { SCORE_RENDERER_CONTRACT_VERSION, type ScoreHighlight, type ScoreMeasureRef, type ScoreNoteRef, type ScorePartRef, type ScoreRenderOptions, type ScoreRenderResult, type ScoreRenderer, type ScoreRendererCapability, type ScoreSource } from "@st/score-renderer-contracts";
...
interface OsmdGraphicalNote { getSVGGElement?(): Element | undefined; getNoteheadSVGs?(): Element[]; vfnoteIndex?: number; sourceNote?: OsmdSourceNote; }
interface OsmdGraphicalVoiceEntry { notes?: readonly OsmdGraphicalNote[]; parentVoiceEntry?: { ParentVoice?: { VoiceId?: number }; parentVoice?: { VoiceId?: number }; }; }
...
export interface OsmdEngine {
  load(content: string): Promise<unknown>; setOptions(options: Record<string, unknown>): void; render(): void; updateGraphic?(): void; cursor?: OsmdCursor;
...
function resolveOpenSheetMusicDisplay(): typeof OsmdModule.OpenSheetMusicDisplay {
  ...
  if (constructor === undefined) throw new Error("OpenSheetMusicDisplay constructor is unavailable from the installed module.");
  return constructor;
}
function createDefaultOsmd(container: HTMLElement): OsmdEngine { const OpenSheetMusicDisplay = resolveOpenSheetMusicDisplay(); return new OpenSheetMusicDisplay(container, { autoResize: true, backend: "svg" }) as unknown as OsmdEngine; }
function requireNonNegativeInteger(value: number, label: string): void { if (!Number.isSafeInteger(value) || value < 0) throw new RangeError(`${label} must be a non-negative safe integer.`); }
function requireFiniteCoordinate(value: number, label: string): void { if (!Number.isFinite(value)) throw new RangeError(`${label} must be a finite number.`); }
...
export class OsmdRenderer implements ScoreRenderer {
  ...
  async render(options: ScoreRenderOptions = {}): Promise<ScoreRenderResult> {
    ...
    await this.clearHighlights(); this.#resetHitTestIndex(); const osmd = this.#ensureOsmd();
    osmd.setOptions({ autoResize: options.autoResize ?? true, drawTitle: options.drawTitle ?? true, drawComposer: options.drawComposer ?? true, pageFormat: options.pageMode === "page" ? "A4 P" : "Endless" });
    osmd.render(); this.#rendered = true;
    try { this.#rebuildHitTestIndex(); } catch (error) { this.#rendered = false; this.#resetHitTestIndex(); throw error; }
  ...
  async highlight(highlight: ScoreHighlight): Promise<void> {
    ...
    if (!HIGHLIGHT_CLASS_PATTERN.test(className)) throw new Error("Highlight className must be one safe CSS class token of at most 64 characters.");
    const element = this.resolveRenderedNoteElement(highlight.target); this.#ensureHighlightStyle(); element.classList.add(className); element.setAttribute("data-st-score-highlight", "true"); this.#highlighted.set(element, className);
  }
  async clearHighlights(): Promise<void> { for (const [element, className] of this.#highlighted) { element.classList.remove(className); element.removeAttribute("data-st-score-highlight"); } this.#highlighted.clear(); }
  async moveCursor(target: ScoreMeasureRef): Promise<void> {
    this.#requireRendered("moveCursor()"); requireNonNegativeInteger(target.measureIndex, "measureIndex"); const osmd = this.#ensureOsmd(); this.#findInstrument(target.partId);
    const measureCount = osmd.Sheet?.SourceMeasures?.length; if (measureCount !== undefined && target.measureIndex >= measureCount) throw new RangeError(`measureIndex ${target.measureIndex} is outside the loaded score.`);
    const cursor = osmd.cursor; if (!cursor) throw new Error("OSMD cursor is unavailable for the loaded score."); cursor.reset(); cursor.show();
    let current = cursor.iterator?.CurrentMeasureIndex; let steps = 0; const maxSteps = (measureCount ?? target.measureIndex + 1) + 2;
    while (current !== target.measureIndex && steps < maxSteps) { if (current !== undefined && current > target.measureIndex) break; if (cursor.nextMeasure) cursor.nextMeasure(); else cursor.next(); current = cursor.iterator?.CurrentMeasureIndex; steps += 1; }
    if (current !== target.measureIndex) throw new Error(`OSMD cursor could not reach measureIndex ${target.measureIndex}.`);
  }
  async setPartVisible(part: ScorePartRef, visible: boolean): Promise<void> {
    if (!this.#loaded) throw new Error("A MusicXML score must be loaded before setPartVisible()."); const osmd = this.#ensureOsmd(); const instrument = this.#findInstrument(part.partId);
    await this.clearHighlights(); this.#resetHitTestIndex(); instrument.Visible = visible; if (!osmd.updateGraphic) throw new Error("OSMD updateGraphic() is unavailable for part visibility changes."); osmd.updateGraphic(); osmd.render(); this.#rendered = true;
    try { this.#rebuildHitTestIndex(); } catch (error) { this.#rendered = false; this.#resetHitTestIndex(); throw error; }
  }
  async dispose(): Promise<void> { await this.clearHighlights(); this.#resetHitTestIndex(); this.#container.replaceChildren(); this.#osmd = undefined; this.#loaded = false; this.#rendered = false; }
  #ensureOsmd(): OsmdEngine { if (this.#osmd === undefined) this.#osmd = this.#factory(this.#container); return this.#osmd; }
  #requireRendered(operation: string): void { if (!this.#rendered) throw new Error(`A score must be rendered before ${operation}.`); }
  #findInstrument(partId: string): OsmdInstrument { if (partId.trim().length === 0) throw new Error("partId must not be empty."); const instrument = this.#ensureOsmd().Sheet?.Instruments?.find((candidate) => candidate.IdString === partId); if (!instrument) throw new Error(`Part '${partId}' was not found in the loaded score.`); return instrument; }
  #listGraphicalNotes(partId: string, measureIndex: number): readonly IndexedGraphicalNote[] {
    const osmd = this.#ensureOsmd(); const instrument = this.#findInstrument(partId); const measure = osmd.graphic?.measureList?.[measureIndex]; if (!measure) throw new Error(`Rendered measure ${measureIndex} is unavailable.`);
    const indexedNotes: IndexedGraphicalNote[] = []; const voiceIndexes = new Map<number, number>(); let globalIndex = 0; const staffIds = instrument.Staves.map((staff) => staff.idInMusicSheet).sort((a, b) => a - b);
    for (const staffId of staffIds) { const graphicalMeasure = measure[staffId]; for (const staffEntry of graphicalMeasure?.staffEntries ?? []) { for (const graphicalVoiceEntry of staffEntry.graphicalVoiceEntries ?? []) { const voice = normalizedVoiceId(graphicalVoiceEntry); for (const note of graphicalVoiceEntry.notes ?? []) { if (voice === undefined) indexedNotes.push(Object.freeze({ note, globalIndex })); else { const voiceIndex = voiceIndexes.get(voice) ?? 0; indexedNotes.push(Object.freeze({ note, globalIndex, voice, voiceIndex })); voiceIndexes.set(voice, voiceIndex + 1); } globalIndex += 1; } } } }
    return indexedNotes;
  }
  #resolveGraphicalNote(target: ScoreNoteRef): OsmdGraphicalNote {
    requireNonNegativeInteger(target.measureIndex, "measureIndex"); requireNonNegativeInteger(target.noteIndex, "noteIndex"); if (target.voice !== undefined) requireNonNegativeInteger(target.voice, "voice");
    const notes = this.#listGraphicalNotes(target.partId, target.measureIndex); const selected = target.voice === undefined ? notes.find((entry) => entry.globalIndex === target.noteIndex) : notes.find((entry) => entry.voice === target.voice && entry.voiceIndex === target.noteIndex);
    if (selected === undefined) { const voiceSuffix = target.voice === undefined ? "" : ` for voice ${target.voice}`; throw new Error(`Rendered note ${target.noteIndex}${voiceSuffix} was not found in part '${target.partId}', measure ${target.measureIndex}.`); }
    return selected.note;
  }
  #isRest(note: OsmdGraphicalNote): boolean { const isRest = note.sourceNote?.isRest; return typeof isRest === "function" && isRest.call(note.sourceNote) === true; }
  #resolveExactNoteheadElement(note: OsmdGraphicalNote): Element {
    if (this.#isRest(note)) throw new Error("The selected score entry is a rest and has no notehead interaction target.");
    const noteheads = note.getNoteheadSVGs?.(); if (!Array.isArray(noteheads) || noteheads.length === 0) throw new Error("OSMD did not expose notehead SVG elements for the selected note.");
    const rawIndex = note.vfnoteIndex; const index = Number.isSafeInteger(rawIndex) && (rawIndex as number) >= 0 ? rawIndex as number : noteheads.length === 1 ? 0 : undefined;
    if (index === undefined) throw new Error("OSMD did not expose a valid notehead index for an ambiguous multi-note graphical entry.");
    const element = noteheads[index]; const ElementConstructor = this.#container.ownerDocument.defaultView?.Element;
