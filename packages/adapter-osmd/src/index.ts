...
const CAPABILITIES: ReadonlySet<ScoreRendererCapability> = new Set([
  "musicxml-render", "svg-export", "cursor", "note-highlight", "part-visibility", "tablature",
]);
...
function sameScoreNoteRef(left: ScoreNoteRef, right: ScoreNoteRef): boolean {
  return left.partId === right.partId && left.measureIndex === right.measureIndex && left.noteIndex === right.noteIndex && left.voice === right.voice;
}
...
export class OsmdRenderer implements ScoreRenderer {
  ...
  async render(options: ScoreRenderOptions = {}): Promise<ScoreRenderResult> {
    ...
    osmd.render();
    this.#rendered = true;
    try { this.#rebuildHitTestIndex(); }
    catch (error) { this.#rendered = false; this.#resetHitTestIndex(); throw error; }
  ...
  async moveCursor(target: ScoreMeasureRef): Promise<void> {
    ...
    if (measureCount !== undefined && target.measureIndex >= measureCount) throw new RangeError(`measureIndex ${target.measureIndex} is outside the loaded score.`);
    ...
    while (current !== target.measureIndex && steps < maxSteps) {
      ...
      if (cursor.nextMeasure) cursor.nextMeasure(); else cursor.next();
  ...
  async setPartVisible(part: ScorePartRef, visible: boolean): Promise<void> {
    ...
    osmd.updateGraphic(); osmd.render(); this.#rendered = true;
    try { this.#rebuildHitTestIndex(); }
    catch (error) { this.#rendered = false; this.#resetHitTestIndex(); throw error; }
  }
  ...
  async dispose(): Promise<void> {
    ...
    this.#osmd = undefined; this.#loaded = false; this.#rendered = false;
  }
      ...
      for (const staffEntry of graphicalMeasure?.staffEntries ?? []) {
        for (const graphicalVoiceEntry of staffEntry.graphicalVoiceEntries ?? []) {
          ...
          for (const note of graphicalVoiceEntry.notes ?? []) {
            if (voice === undefined) indexedNotes.push(Object.freeze({ note, globalIndex }));
            else {
  ...
  #resolveExactNoteheadElement(note: OsmdGraphicalNote): Element {
    ...
    const index = note.vfnoteIndex;
    if (!Number.isSafeInteger(index) || (index as number) < 0) throw new Error("OSMD did not expose a valid notehead index for the selected note.");
    const noteheads = note.getNoteheadSVGs?.();
    if (!Array.isArray(noteheads)) throw new Error("OSMD did not expose notehead SVG elements for the selected note.");
    const element = noteheads[index as number];
    if (!(element instanceof this.#container.ownerDocument.defaultView!.Element)) throw new Error("OSMD did not expose an exact SVG notehead element for the selected note.");
    return element;
  }
    ...
    for (const instrument of instruments) {
      for (let measureIndex = 0; measureIndex < measureCount; measureIndex += 1) {
        ...
        for (const entry of entries) {
          if (this.#isRest(entry.note)) continue;
          const element = this.#resolveExactNoteheadElement(entry.note);
          indexedElementCount += 1;
          if (indexedElementCount > MAX_HIT_TEST_NOTE_ELEMENTS) throw new RangeError(`Rendered note hit-test index exceeds ${MAX_HIT_TEST_NOTE_ELEMENTS} elements.`);
          const target: ScoreNoteRef = entry.voice === undefined
            ...
            : Object.freeze({ partId: instrument.IdString, measureIndex, noteIndex: entry.voiceIndex as number, voice: entry.voice });
  ...
  #registerHitTestElement(element: Element, target: ScoreNoteRef): void {
    if (!this.#noteRefByElement.has(element)) { this.#noteRefByElement.set(element, target); return; }
